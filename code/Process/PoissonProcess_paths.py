...
def GeneratePathsPoisson(NoOfPaths,NoOfSteps,T,xiP):    
    ...
    X = np.zeros([NoOfPaths, NoOfSteps+1]) # Poisson process
    Xc = np.zeros([NoOfPaths, NoOfSteps+1]) # compensated Poisson process
...
def mainCalculation(NoOfPaths, NoOfSteps, T, xiP):
    ...
    plt.title("Poisson Process X(t) Paths")
    plt.grid()
    plt.xlabel("time")
    plt.ylabel("X(t)")
    
    plt.figure(2)
    plt.plot(timeGrid, np.transpose(Xc))   
    plt.title("Compensated Poisson Process X(t) Paths")
    plt.grid()
    plt.xlabel("time")
    plt.ylabel("X(t)")
