...
def COSDensity(cf,x,N,a,b):
    i = np.complex128(0.0,1.0) #assigning i=sqrt(-1)
...
def mainCalculation():
    ...
    plt.figure(1)
    plt.title(f"Merton Process log-price X(t) Paths")
    ...
    plt.figure(2)
    plt.title(f"Merton Process spot price S(t) Paths")
    ...
    sns.kdeplot(x=X[:, -1], color='darkblue', linewidth=3)
    ...
    plt.title(f"Merton Process log-price X(t) for paths and density")
    ...
    plt.figure(5)
    ax = plt.axes(projection='3d')
    plt.title(f"Merton Process spot price S(t) for paths and density")
    zline = np.zeros([len(timeGrid),1])
